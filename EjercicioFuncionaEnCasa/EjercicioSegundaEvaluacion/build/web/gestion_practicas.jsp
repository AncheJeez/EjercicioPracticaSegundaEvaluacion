...
<html>
    ...
    <body>
        ...
        <div class="container text-center mt-5">
            ...
            <c:if test="${empty practicas}">
                <div class="alert alert-warning" role="alert">
                    No hay prácticas registradas en el sistema.
                </div>
            </c:if>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <c:forEach var="practica" items="${practicas}">
                        <tr>
                            <td>${practica.alumno.nombre} ${practica.alumno.apellidos}</td>
                            <td>${practica.empresa.nombre}</td>
                            <td>${practica.fecha_comienzo}</td>
                            <td>${practica.fecha_finalizacion}</td>
                            <td>${practica.comentarios}</td>
                            <td>
                                <a href="ServletGestionPracticas?id=${practica.id_practica}" class="btn btn-warning btn-sm">Editar</a>

                                <a href="ServletGestionPracticas?action=borrar&id=${practica.id_practica}" 
                                   class="btn btn-danger btn-sm"
                                   onclick="return confirm('¿Estás seguro de que deseas borrar esta practica?');">
                                   Borrar
                                </a>
                            </td>
                        </tr>
            ...
            <a href="ServletGestionPracticas?action=new" class="btn btn-primary btn-sm">Crear Practica</a>
        </div>
    </body>
</html>
